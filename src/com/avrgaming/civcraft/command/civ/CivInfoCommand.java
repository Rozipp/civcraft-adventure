...
public class CivInfoCommand extends CommandBase {
	...
	public void init() {
		...
		cs.add("upkeep", CivSettings.localize.localizedString("cmd_civ_info_upkeepDesc"));
		cs.add("taxes", CivSettings.localize.localizedString("cmd_civ_info_taxesDesc"));
		cs.add("beakers", CivSettings.localize.localizedString("cmd_civ_info_beakersDesc"));
		cs.add("online", CivSettings.localize.localizedString("cmd_civ_info_onlineDesc"));
	}
	...
	public void upkeep_cmd() throws CivException {
		...
		if (args.length < 2) {
			...
			for (Town town : civ.getTowns()) {
				CivMessage.send(sender, CivColor.Green + CivSettings.localize.localizedString("Town") + " " + CivColor.LightGreen + town.getName() + CivColor.Green + CivSettings.localize.localizedString("Total") + " " + CivColor.LightGreen
						+ getTownTotalLastTick(town, civ));
			}
	...
	public static void show(CommandSender sender, Resident resident, Civilization civ) {
		...
		if (civ.GM.getLeaderGroup() == null) {
			...
			CivMessage.send(sender, CivColor.Green + civ.GM.leaderGroupName + ": " + CivColor.LightGreen + civ.GM.getLeaderGroup().getMembersString());
		}
