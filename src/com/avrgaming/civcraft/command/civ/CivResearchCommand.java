...
public class CivResearchCommand extends CommandBase {
	...
	public void queueadd_cmd() throws CivException {
		...
		if (this.args.length < 2) throw new CivException(CivSettings.localize.localizedString("cmd_civ_research_queueEnterName"));
		if (!civ.getCapitol().isValid()) throw new CivException(CivSettings.localize.localizedString("cmd_civ_research_queueTownHallNULL"));
		if (!civ.getCapitolCityHall().isActive()) throw new CivException(CivSettings.localize.localizedString("cmd_civ_research_queueNotCompletedTownHall"));
		...
		if (civ.getResearchTech() == null) throw new CivException(CivSettings.localize.localizedString("cmd_civ_research_queueNoResearchingNow", tech.name));
