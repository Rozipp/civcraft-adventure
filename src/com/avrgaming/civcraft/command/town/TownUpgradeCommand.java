...
public class TownUpgradeCommand extends CommandBase {
	...
	public void init() {
		...
		cs.add("list", CivSettings.localize.localizedString("cmd_town_upgrade_listDesc"));
		cs.add("purchased", CivSettings.localize.localizedString("cmd_town_upgrade_purchasedDesc"));
		cs.add("buy", CivSettings.localize.localizedString("cmd_town_upgrade_buyDesc"));
			...
			if (category.equalsIgnoreCase("all") || upgrade.category.equalsIgnoreCase(category)) {
				if (upgrade.isAvailable(town)) {
					CivMessage.send(sender,
							upgrade.name + " " + CivColor.LightGray + CivSettings.localize.localizedString("Cost") + " " + CivColor.Yellow + upgrade.cost);
				}
			}
		}
	}
	...
	public void list_cmd() throws CivException {
		...
		if (args.length < 2) {
			CivMessage.send(sender, "- " + CivColor.Gold + CivSettings.localize.localizedString("cmd_town_upgrade_listAllHeading") + " " + CivColor.LightBlue
					+ "(" + ConfigTownUpgrade.getAvailableCategoryCount("all", town) + ")");
			for (String category : ConfigTownUpgrade.categories.keySet()) {
				CivMessage.send(sender, "- " + CivColor.Gold + WordUtils.capitalize(category) + CivColor.LightBlue + " ("
						+ ConfigTownUpgrade.getAvailableCategoryCount(category, town) + ")");
			}
			return;
		}
