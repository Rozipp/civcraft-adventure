		...
		try {
			if (selection == null) {
				if (message.equalsIgnoreCase("town")) {
					...
					if (message.equalsIgnoreCase("civ")) {
				...
				if (oldName == null) {
					...
					if (selection.equals("town")) {
						...
						if (!town.GM.getMayorGroup().hasMember(resident) && !town.getCiv().GM.getLeaderGroup().hasMember(resident)) {
						...
						if (selection.equals("civ")) {
							...
							if (!civ.GM.isLeader(resident)) throw new CivException(CivSettings.localize.localizedString("interactive_rename_civnoPerms"));
