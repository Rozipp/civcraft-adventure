...
public class BonusGoodie extends LoreItem {
	/* This class represents a lore item that is tied to a trade outpost. We need to keep track of its location at all times so we can despawn
	 * it. */
	public static enum LoreIndex {
		TYPE, OUTPOSTLOCATION
	}
	...
	/* NOTE: This item pointer will probably be invalid after a chunk unload and reload. For now it's OK since we do repo goodies that are left
	 * on the ground when the chunks unload. But if we extend this in the future, we will need to store the Item's UUID and grab it like we do
	 * for frames. */
	...
	public static void init() throws SQLException {
		if (!SQL.hasTable(TABLE_NAME)) {
			String table_create = "CREATE TABLE " + SQL.tb_prefix + TABLE_NAME + " (" + "`id` int(11) unsigned NOT NULL auto_increment," + "`holder_location` mediumtext DEFAULT NULL," + "`player_name` mediumtext DEFAULT NULL,"
					+ "`frame_location` mediumtext DEFAULT NULL," + "`frame_uid` mediumtext DEFAULT NULL," + "`item_uid` mediumtext DEFAULT NULL," + "`outpost_location` mediumtext DEFAULT NULL," + "PRIMARY KEY (`id`)" + ")";
	...
	/* Loads a bonus goodie from the outpost location, this is the primary load function that is called. This function calls load with the
	 * result set. Bonus goodies are loaded as the trade outposts are loaded. */
	...
	private void createGoodieAtOutpost() throws CivException {
		...
		ItemStack stack = ItemManager.createItemStack(this.config.material, (short) this.config.material_data, 1);
	...
	public void updateLore(ItemStack stack) {
		...
		if (isStackable()) {
			lore.add(CivColor.LightBlue + CivSettings.localize.localizedString("stackable"));
		} else {
			lore.add(CivColor.LightGray + CivSettings.localize.localizedString("notStackable"));
		}
	...
	/* Despawns the item from its current location and puts it back in the itemframe on the outpost which it belongs to. */
	...
	/* De-spawns an item from its current location, and puts it back in the item frame on the outpost which it belongs to. */
	...
	/* Updates the internal location data of the goodie and saves it to the database if needed. */
	public void update(boolean sync) throws CivException {
		...
		if (holderStore != null) {
			if (holderStore.getHolder() instanceof Chest) {
				...
				if (holderStore.getHolder() instanceof DoubleChest) {
					...
					if (holderStore.getHolder() instanceof Player) {
	...
	private boolean isItemStackOurs(ItemStack stack) {
		...
		if (meta.hasLore() && meta.getLore().size() >= LoreIndex.values().length) {
			if (meta.getLore().get(LoreIndex.TYPE.ordinal()).equals(BonusGoodie.LORE_TYPE)) {
				...
				if (loc.getBlockX() == outpost.getCorner().getX() && loc.getBlockY() == outpost.getCorner().getY() && loc.getBlockZ() == outpost.getCorner().getZ()) {
					// Found ya!
					return true;
				}
			}
	...
	public boolean equals(Object other) {
		if (other instanceof BonusGoodie) {
			...
			if (otherCoord.getOutpost().getCorner().toString().equals(this.getOutpost().getCorner().toString())) {
				return true;
			}
		}
		return false;
