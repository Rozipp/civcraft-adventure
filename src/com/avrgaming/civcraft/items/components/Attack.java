...
//        ConfigUnit artifact = UnitStatic.getPlayerUnit((Player)event.getDamager());
		double extraAtt = 0.0;
		for (LoreEnhancement loreEnhancement : attrs.getEnhancements()) {
			...
			if (loreEnhancement instanceof LoreEnhancementPoison && !event.isCancelled() && event.getEntity() instanceof LivingEntity
					&& !(event.getEntity() instanceof ArmorStand) && !(event.getEntity() instanceof Slime)) {
			...
			if (loreEnhancement instanceof LoreEnhancementLevitate && !event.isCancelled() && event.getEntity() instanceof LivingEntity
					&& !(event.getEntity() instanceof ArmorStand) && !(event.getEntity() instanceof Slime) && event.getEntity() instanceof Player
			...
			if (!(loreEnhancement instanceof LoreEnhancementLightningStrike) || event.isCancelled() || lightning_chance > 55000) continue;
			if (event.getEntity() instanceof LivingEntity && !(event.getEntity() instanceof ArmorStand) && !(event.getEntity() instanceof Slime)) {
				...
				CivMessage.send((Object) event.getDamager(),
						CivColor.LightBlueItalic + CivSettings.localize.localizedString("loreEnh_LightStrike_Sucusses", arrobject));
				if (!(toDamage instanceof Player)) continue;
				CivMessage.send((Object) event.getEntity(),
						CivSettings.localize.localizedString("loreEnh_LightStrike_Sucusses2", event.getDamager().getName()));
				continue;
			}
