...
public class ShowRecipe implements GuiAction {
	...
	public ItemStack getIngredItem(ConfigIngredient ingred, Inventory recInv) {
		...
		if (ingred.custom_id == null) {
			...
			name = cmat.getName();
			entryStack = CraftableCustomMaterial.spawn(cmat);
	...
	public void buildCraftTableBorder(Inventory recInv) {
		...
		stack = LoreGuiItem.build("Craft Table Border", ItemManager.getMaterialId(Material.WORKBENCH), 0, "");
	...
	public void buildInfoBar(CraftableCustomMaterial craftMat, Inventory recInv, Player player) {
		int offset = 0;
		...
		if (craftMat.getConfigMaterial().required_tech != null) {
			...
			if (tech != null) {
				...
				if (resident.hasTown() && resident.getCiv().hasTechnology(craftMat.getConfigMaterial().required_tech)) {
					stack = LoreGuiItem.build(CivSettings.localize.localizedString("loreGui_recipes_requiredTech"), ItemManager.getMaterialId(Material.EMERALD_BLOCK), 0, tech.name);
				} else {
					stack = LoreGuiItem.build(CivSettings.localize.localizedString("loreGui_recipes_requiredTech"), ItemManager.getMaterialId(Material.REDSTONE_BLOCK), 0, tech.name);
				}
			}
		...
		if (craftMat.isShaped()) {
			stack = LoreGuiItem.build(CivSettings.localize.localizedString("loreGui_recipes_shaped"), ItemManager.getMaterialId(Material.HOPPER), 0, "");
		} else {
			stack = LoreGuiItem.build(CivSettings.localize.localizedString("loreGui_recipes_unshaped"), ItemManager.getMaterialId(Material.COAL), 0, "");
		}
	...
	public void performAction(InventoryClickEvent event, ItemStack stack) {
		...
		if (backInventory != null) {
			...
			ItemStack backButton = LoreGuiItem.build(CivSettings.localize.localizedString("loreGui_recipes_back"), ItemManager.getMaterialId(Material.MAP), 0, CivSettings.localize.localizedString("loreGui_recipes_back"));
			...
			if (cat != null) {					
				ItemStack backButton = LoreGuiItem.build(CivSettings.localize.localizedString("loreGui_recipes_back"), ItemManager.getMaterialId(Material.MAP), 0, CivSettings.localize.localizedString("loreGui_recipes_backMsg")+" "+cat.name);
				backButton = LoreGuiItem.setAction(backButton, "OpenInventory");
				backButton = LoreGuiItem.setActionData(backButton, "invType", "showGuiInv");
