    ...
    public void performAction(InventoryClickEvent event, ItemStack stack) {
        ...
        for (ConfigTownUpgrade upgrade : ConfigTownUpgrade.getAllUpgrades(town)) {
            ...
            if (upgrade.isAvailable(town)) {
                ...
                is = LoreGuiItem.build(upgrade.name, ItemManager.getMaterialId(Material.GLOWSTONE_DUST), 0, "§b" + CivSettings.localize.localizedString("money_requ", Math.round(cost)), CivColor.Red + CivSettings.localize.localizedString("requ") + upgrade1.name, "§3" + CivSettings.localize.localizedString("tutorial_lore_clicktoView"));
                is = LoreGuiItem.setAction(is, "UpgradeGuiBuy");
        ...
        ItemStack backButton = LoreGuiItem.build(CivSettings.localize.localizedString("bookReborn_back"), ItemManager.getMaterialId(Material.MAP), 0, CivSettings.localize.localizedString("bookReborn_backToDashBoard"));
