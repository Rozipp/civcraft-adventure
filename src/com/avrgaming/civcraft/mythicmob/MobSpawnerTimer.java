...
//				if (!world.getAllowMonsters()) continue;
				...
				for (int j = 0; j < getMinSpawnAmount(Bukkit.getOnlinePlayers().size()); j++) {
					...
					int y = world.getHighestBlockYAt(((Double) player.getLocation().getX()).intValue() + x,
							((Double) player.getLocation().getZ()).intValue() + z);
					...
					if ((ItemManager.getTypeId(loc.getBlock().getRelative(BlockFace.DOWN)) == CivData.WATER)
							|| (ItemManager.getTypeId(loc.getBlock().getRelative(BlockFace.DOWN)) == CivData.WATER_RUNNING)
							|| (ItemManager.getTypeId(loc.getBlock().getRelative(BlockFace.DOWN)) == CivData.LAVA)
							|| (ItemManager.getTypeId(loc.getBlock().getRelative(BlockFace.DOWN)) == CivData.LAVA_RUNNING)) {
						continue;
					}
					...
					MobStatic.spawnRandomCustomMob(loc);
				}
				break;
			} catch (CivException e) {} finally {
				if (name != null) {
	...
	public int getMinSpawnAmount(int onlinePlayers) {
		if (onlinePlayers > 75) {
			...
			if (onlinePlayers < 75 && onlinePlayers > 50) {
				return 45;
			} else
				if (onlinePlayers < 50 && onlinePlayers > 25) {
					return 30;
				}
