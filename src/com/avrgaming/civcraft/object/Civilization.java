	...
	public static void init() throws SQLException {
		if (!SQL.hasTable(TABLE_NAME)) {
			String table_create = "CREATE TABLE " + SQL.tb_prefix + TABLE_NAME+" (" + 
					...
					"`conquered_date` long,"+
				    "`created_date` long," +
				    "`ownerGroupName` mediumtext DEFAULT NULL," + // названние группы основателя цивилизации
...
//		Resident res = CivGlobal.getResidentViaUUID(UUID.fromString(resUUID));
	...
	public static void newCiv(String name, String capitolName, String tag, Resident resident,
		...
		if (getResearchProgress() >= getResearchTech().getAdjustedBeakerCost(this)) {
            ...
            if (this.getTechQueue() != null) {
                ...
                if (!this.getTreasury().hasEnough(tech.getAdjustedTechCost(this))) {
                    CivMessage.sendTechError(this, CivSettings.localize.localizedString("civ_research_queueErrorCodeThree", tech.name, tech.getAdjustedTechCost(this) - this.getTreasury().getBalance(), Resident.plurals((int)(tech.getAdjustedTechCost(this) - this.getTreasury().getBalance()), "монета", "монеты", "монет")));
                    return;
                }
                if (this.getResearchTech() != null) {
