...
public class DiplomacyManager {
	 ...
	 * will be duplicate data that needs to be cleaned up for mutual relationships. For example, if Civ A is at war with Civ B, both Civ A and Civ B will have a
	 * war relationship entry. */

	private Civilization ourCiv;
	private int coalitionId = 0;
	...
	/** есть ли с кем то война */
	public boolean isAtWar() {
		return (warCount != 0);
	}

	public void deleteRelation(Relation relation) {
		if (relation.getStatus() == Relation.Status.WAR && relations.containsKey(relation.getOtherCiv().getId())) {
	...
	public void setRelation(Civilization otherCiv, Relation.Status status, Date expires) {
		...
		} else {//Если были какие то отношения, то ...
			if (relation.getStatus() == status) return;
			if (relation.getStatus() == Relation.Status.WAR) warCount--;
			if (expires != null) relation.setExpires(expires);
...
//			}
		...
		if (status == Relation.Status.WAR) warCount++;
	}

	public Relation.Status getRelationStatus(Civilization otherCiv) {
		if (otherCiv.getId() == ourCiv.getId()) return Relation.Status.ALLY;

		Relation relation = getRelation(otherCiv);
		if (relation == null) return Relation.Status.NEUTRAL;
	...
	public void addRelation(Relation relation) {
		...
		if (relation.getStatus() == Relation.Status.WAR) {
			if (currentRelation == null || currentRelation.getStatus() != Relation.Status.WAR) warCount++;
		}
...
//	}
	...
	public boolean isCoalitionWith(Resident resident) {
		return isCoalitionWith(resident.getCiv());
	}
	
	public boolean isCoalitionWith(Civilization civ) {
		Relation relation = this.relations.get(civ.getId());
		if (relation.getStatus() != Relation.Status.ALLY) return false;
		return (this.getCoalitionId() == civ.getDiplomacyManager().getCoalitionId()); 
	}
}
