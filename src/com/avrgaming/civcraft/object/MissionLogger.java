...
public class MissionLogger {
	...
	public static void init() throws SQLException {
		if (!SQL.hasTable(TABLE_NAME)) {
			String table_create = "CREATE TABLE " + SQL.tb_prefix + TABLE_NAME + " (" + "`id` int(11) unsigned NOT NULL auto_increment," + "`town_id` int(11) unsigned DEFAULT 0," + "`target_id` int(11) unsigned DEFAULT 0," + "`time` long,"
					+ "`playerName` mediumtext," + "`missionName` mediumtext," + "`result` mediumtext," + "PRIMARY KEY (`id`)" + ")";
	...
	public static void logMission(Town town, Town target, Resident resident, String missionName, String result) {
		...
		hashmap.put("playerName", resident.getUuid().toString());
