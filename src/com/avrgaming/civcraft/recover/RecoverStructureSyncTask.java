...
public class RecoverStructureSyncTask implements Runnable {
		...
		for (int x = 0; x < tpl.size_x; x++) {
			for (int y = 0; y < tpl.size_y; y++) {
				for (int z = 0; z < tpl.size_z; z++) {
					...
					if (ItemManager.getTypeId(nextBlock) != CivData.BEDROCK) {
	...
	public void run() {
		for (Structure struct : this.structures) {
			CivMessage.send(sender, CivSettings.localize.localizedString("structureRepairStart") + " " + struct.getDisplayName() + " @ " + CivColor.Yellow
					+ struct.getCorner());
