...
public class CannonProjectile {
							...
							if (!structuresHit.contains(sb.getOwner())) {
								...
								if (sb.getOwner() instanceof Townhall) {
									...
									CivMessage.sendCiv(sb.getCiv(),
											CivColor.Yellow
													+ CivSettings.localize.localizedString("cannonProjectile_hitAnnounce", sb.getOwner().getDisplayName(),
															sb.getOwner().getCenterLocation().getX() + "," + sb.getOwner().getCenterLocation().getY() + ","
																	+ sb.getOwner().getCenterLocation().getZ())
													+ " (" + sb.getOwner().getHitpoints() + "/" + sb.getOwner().getMaxHitPoints() + ")");
								}

								CivMessage.sendCiv(whoFired.getCiv(),
										CivColor.LightGreen
												+ CivSettings.localize.localizedString("var_cannonProjectile_hitSuccess", ((Buildable)sb.getOwner()).getTown().getName(),
														sb.getOwner().getDisplayName())
												+ " (" + sb.getOwner().getHitpoints() + "/" + sb.getOwner().getMaxHitPoints() + ")");
							}
						} else {
							if (!Cannon.cannonBlocks.containsKey(bcoord)) explodeBlock(b);
						}
					}
				}
			}
	...
	LinkedList<Entity> players = EntityProximity.getNearbyEntities(null, loc, yield, EntityPlayer.class);for(
	Entity e:players)
	{
		...
		if (player.isDead()) {
			CivMessage
					.global(CivColor.LightGray + CivSettings.localize.localizedString("var_cannonProjectile_userKilled", player.getName(), whoFired.getName()));
		}
	}
	}
	...
	public boolean advance() {
		...
		if (ItemManager.getTypeId(loc.getBlock()) != CivData.AIR) {
			return true;
		}
