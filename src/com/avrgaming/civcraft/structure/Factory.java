...
public class Factory extends Structure
{
    ...
    public void trainCraftMat(final CivAsyncTask task, final Player player) throws InterruptedException {
        ...
        String[] chestIds = {"0", "1", "2", "3", "4", "5", "6"}; 
        final Collection<StructureChest> chests = this.getAllChestsById(chestIds);
        ...
        if (!allMatchMinecraft) {
            for (final ItemStack itemStack : source.getContents()) {
                if (itemStack != null) {
                    final CustomMaterial loreMaterial = CustomMaterial.getCustomMaterial(itemStack);
                    if (loreMaterial == null) {
                        final Integer id = ItemManager.getTypeId(itemStack);
        ...
        final CraftableCustomMaterial craftMat = CraftableCustomMaterial.getCraftableCustomMaterial(configSpaceCraftMat.originalCraftMat);
        ...
        if (allAllMatch) {
            ...
            final ItemStack stack = CustomMaterial.spawn(craftMat);
    ...
    private void changeIndex(final int newIndex) {
        ...
        if (this.trainSign != null) {
            try {
                final CraftableCustomMaterial craftMat = CraftableCustomMaterial.getCraftableCustomMaterial(crafts.get(newIndex).originalCraftMat);
                ...
                if (crafts.size() > 0) {
                    final CraftableCustomMaterial craftMat = CraftableCustomMaterial.getCraftableCustomMaterial(crafts.get(0).originalCraftMat);
                    this.trainSign.setText(CivSettings.localize.localizedString("structure_factory_sign_construct") + "\n" + CivColor.GreenBold + craftMat.getName());
                    this.index = 0;
                }
            }
