...
import com.avrgaming.civcraft.structure.MagicShip;
...
public class MagicTower extends Structure {
	...
	public void loadSettings() {
		super.loadSettings();
		this.magicComponent = new ProjectileMagicComponent(this, this.getCenterLocation());
	...
	public void onCheck() throws CivException {
		try {
			double build_distanceSqr = Math.pow(CivSettings.getDouble(CivSettings.warConfig, "tesla_tower.build_distance"), 2);
			for (Town town : this.getTown().getCiv().getTowns()) {
				for (Structure structure : town.getStructures()) {
					if (structure instanceof MagicTower) {
						Location center = structure.getCenterLocation();
						double distanceSqr = center.distanceSquared(this.getCenterLocation());
						if (distanceSqr <= build_distanceSqr)
							throw new CivException(CivSettings.localize.localizedString("var_buildable_tooCloseToMagicTower",
									"" + center.getX() + "," + center.getY() + "," + center.getZ()));
					}
					if (structure instanceof MagicShip) {
						Location center = structure.getCenterLocation();
						double distanceSqr = center.distanceSquared(this.getCenterLocation());
						if (distanceSqr <= build_distanceSqr)
							throw new CivException(CivSettings.localize.localizedString("var_buildable_tooCloseToMagicShip",
									"" + center.getX() + "," + center.getY() + "," + center.getZ()));
					}
				}
			}
