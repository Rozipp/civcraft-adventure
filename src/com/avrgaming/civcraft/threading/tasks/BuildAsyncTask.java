...
public class BuildAsyncTask extends CivAsyncTask {
				...
				if (nextPercentComplete > this.percent_complete) {
					...
					if ((this.percent_complete % 10 == 0)) {
						if (this.buildable instanceof Wonder) {
							...
							CivMessage.global(CivSettings.localize.localizedString("var_buildAsync_progressWonder", this.buildable.getDisplayName(), this.buildable.getTown().getName(), nextPercentComplete, this.buildable.getCiv().getName()));
                        }
						else 
						{
	...
	public boolean build() {
		...
		int n = buildable.builtBlockCount % (tpl.size_x * tpl.size_z);
		...
		int y = (buildable.builtBlockCount / (tpl.size_x * tpl.size_z)); //bottom to top.
		if (q < p) {
			z = z + k * q;
		} else {
			x = x - k * (q - p);
