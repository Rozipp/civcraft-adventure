...
public class CultureProcessAsyncTask extends CivAsyncTask {
	...
	private void processTownCulture(Town town) {
		...
		if (expandedAmount > 0) {
			CivMessage.sendCiv(town.getCiv(), CivSettings.localize.localizedString("var_cultureProcess_townGrow", town.getName(), expandedAmount));
		}
	...
	private void _processCultureBreadthFirst(Town town, ChunkCoord origin, CultureChunk starting, HashSet<ChunkCoord> expanded) {
		...
		while (openList.isEmpty() == false) {
			...
			if (closedChunks.containsKey(node.getChunkCoord())) continue;
			...
			for (int i = 0; i < 4; i++) {
				ChunkCoord nextCoord = node.getChunkCoord().getRelative(offset[i][0], offset[i][1]);
				...
				if (neighbor == null) {
					...
					if (neighbor.getTown() != node.getTown()) {
						...
						/* This is our chunk, but we should add it to expanded anyway so we can remove unused culture chunks. */
				...
				if (townChunk != null) {
					if (townChunk.getTown() == town) neighbor.setDistance(0);
				}
	...
	private static void processStructureFlipping() {
		/* Now that all of the towns have had their say on what chunks belong to who, now we need to determine if any structures belonging to
		 * another town are in the wrong spot. */
	...
	private static void recalculateTouchingCultures() {
		for (Town t : CivGlobal.getTowns()) {
			...
			for (CultureChunk cc : t.getCultureChunks()) {
				...
				int[][] offset = { { -1, 0 }, { 1, 0 }, { 0, -1 }, { 0, 1 } };
				for (int i = 0; i < 4; i++) {
					ChunkCoord nextCoord = cc.getChunkCoord().getRelative(offset[i][0], offset[i][1]);

					CultureChunk next = CivGlobal.getCultureChunk(nextCoord);
					if (next == null) continue;
					if (next.getTown() == cc.getTown()) continue;
					if (t.townTouchList.contains(next.getTown())) continue;
