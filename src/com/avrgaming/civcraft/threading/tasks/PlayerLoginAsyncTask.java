		...
		try {
			...
			if (resident == null) {
				...
				try {
					...
					TaskMaster.syncTask(new PlayerKickBan(getPlayer().getName(), true, false,
							CivSettings.localize.localizedString("PlayerLoginAsync_usernameInvalid")));
					return;
				}
				...
				CivMessage.send(resident, CivColor.LightGray
						+ CivSettings.localize.localizedString("var_PlayerLoginAsync_pvpTimerPropmt1", mins));
				CivMessage.send(resident,
						CivColor.LightGray + CivSettings.localize.localizedString("PlayerLoginAsync_pvpTimerPropmt2"));
			}

			/*
			 * Resident is present. Lets check the UUID against the stored UUID. We are not
			 * going to allow residents to change names without admin permission. If someone
			 * logs in with a name that does not match the stored UUID, we'll kick them.
			 */
			if (resident.getUid() == null) {
				/* This resident does not yet have a UUID stored. Free lunch. */
				resident.setUid(getPlayer().getUniqueId());
				CivLog.info("Resident named:" + resident.getName() + " was acquired by UUID:"
						+ resident.getUid().toString());
			} else if (!resident.getUid().equals(getPlayer().getUniqueId())) {
				TaskMaster.syncTask(new PlayerKickBan(getPlayer().getName(), true, false,
						CivSettings.localize.localizedString("PlayerLoginAsync_usernameInUse")));
				return;
			}
			...
			if (War.isWarTime() && War.isOnlyWarriors()) {
				if (!getPlayer().isOp() || getPlayer().hasPermission(CivSettings.MINI_ADMIN)) {
					if (!resident.hasTown() || !resident.getTown().getCiv().getDiplomacyManager().isAtWar()) {
						TaskMaster.syncTask(new PlayerKickBan(getPlayer().getName(), true, false,
								CivSettings.localize.localizedString("PlayerLoginAsync_onlyWarriorsAllowed")));
						return;
					}
			...
			if (!getPlayer().isOp()) {
				...
				if (cc != null && cc.getCiv() != resident.getCiv()) {
					...
					if (War.isWarTime() && status.equals(Relation.Status.WAR) && !cc.getCiv().isAdminCiv()) {
						/*
						 * Test for players who were not logged in when war time started. If they were
						 * not logged in, they are enemies, and are inside our borders they need to be
						 * teleported back to their own town hall.
						 */
						...
						resident.teleportHome();
						CivMessage.send(resident, CivColor.LightGray
								+ CivSettings.localize.localizedString("PlayerLoginAsync_loginNotAllies"));
					}

					CivMessage.sendCiv(cc.getCiv(),
							CivSettings.localize.localizedString("var_PlayerLoginAsync_inBorderAlert",
									(color + getPlayer().getDisplayName() + "(" + relationName + ")")));
				}
			}
			...
			try {
				...
				ArrayList<SessionEntry> deathEvents = CivGlobal.getSessionDatabase()
						.lookup("pvplogger:death:" + resident.getName());
				if (deathEvents.size() != 0) {
					CivMessage.send(resident, CivColor.Rose + CivColor.BOLD
							+ CivSettings.localize.localizedString("PlayerLoginAsync_killedWhilePVPLogged"));
			...
			if (EndConditionDiplomacy.canPeopleVote()) {
				CivMessage.send(resident,
						CivColor.LightGreen + CivSettings.localize.localizedString("PlayerLoginAsync_councilOf8"));
			}
			Civilization civ = resident.getCiv();
			if (civ != null) {
				if (civ.MOTD() != null) {
					CivMessage.send(resident,
							CivColor.LightPurple + "[Civ MOTD] " + CivColor.White + resident.getCiv().MOTD());
				}
			}
