...
public class QuarryAsyncTask extends CivAsyncTask {
	...
	public void processQuarryUpdate() {
		...
		for (int i = 0; i < quarry.skippedCounter + 1; i++) {
			for (ItemStack stack : contents) {
				...
				if (ItemManager.getTypeId(stack) == CivData.WOOD_PICKAXE) {
					try {
				...
				if (this.quarry.getLevel() >= 2 && ItemManager.getTypeId(stack) == CivData.STONE_PICKAXE) {
					try {
				...
				if (this.quarry.getLevel() >= 3 && ItemManager.getTypeId(stack) == CivData.IRON_PICKAXE) {
					try {
						short damage = ItemManager.getData(stack);
						this.updateInventory(Action.REMOVE, source_inv, stack);
						damage += modifier;
					...
					if (rand1 < ((int) ((quarry.getChance(Mineral.RARE)) * randMax))) {
						...
						newItem = CustomMaterial.spawn(CustomMaterial.getCustomMaterial("mat_tungsten_ore"), modifier);
				...
				if (ItemManager.getTypeId(stack) == CivData.GOLD_PICKAXE) {
					try {
						short damage = ItemManager.getData(stack);
						this.updateInventory(Action.REMOVE, source_inv, stack);
						damage += modifier;
				...
				if (this.quarry.getLevel() >= 4 && ItemManager.getTypeId(stack) == CivData.DIAMOND_PICKAXE) {
					try {
						short damage = ItemManager.getData(stack);
						this.updateInventory(Action.REMOVE, source_inv, stack);
						damage += modifier;
	...
	private ItemStack getJunk(int modifier) {
		...
		if (rand2 < (2)) {
			return ItemManager.createItemStack(CivData.DIRT, (short) CivData.PODZOL, modifier);
		} else if (rand2 < (5)) {
			return ItemManager.createItemStack(CivData.DIRT, (short) CivData.COARSE_DIRT, modifier);
		} else {
	...
	private ItemStack getOther(int modifier) {
		...
		int rand2 = rand.nextInt(randMax);
		if (rand2 < (randMax / 8)) {
			return ItemManager.createItemStack(CivData.STONE, (short) CivData.ANDESITE, modifier);
		} else if (rand2 < (randMax / 5)) {
			return ItemManager.createItemStack(CivData.STONE, (short) CivData.DIORITE, modifier);
		} else {
			return ItemManager.createItemStack(CivData.STONE, (short) CivData.GRANITE, modifier);
		}
	}
			...
			try {
				try {
	                ...
	                if (CivData.randChance(this.quarry.getTown().getReturnChance())) {
	                    processQuarryUpdate();
	                }//TODO WARNNING Что  ето запроверка !!!!! ЗАчем второй раз запускать карьер??? 
				} catch (Exception e) {
					e.printStackTrace();
				}
			} finally {
