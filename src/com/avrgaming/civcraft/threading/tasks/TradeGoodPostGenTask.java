	...
	class SyncTradeGenTask implements Runnable {
		...
		public void run() {
			for (int i = 0; i < amount; i++) {
				...
				Chunk chunk = CivCraft.mainWorld.getChunkAt(coord.getX(), coord.getZ());

				int centerX = (chunk.getX() << 4) + 8;
				int centerZ = (chunk.getZ() << 4) + 8;
				int centerY = CivCraft.mainWorld.getHighestBlockYAt(centerX, centerZ);

				BlockCoord bcoord = new BlockCoord(CivCraft.mainWorld, centerX, centerY - 1, centerZ);

				/* try to detect already existing trade goods. */
				while (true) {
					Block top = bcoord.getBlock();
					...
					if (ItemManager.getTypeId(top) == CivData.BEDROCK) {
						...
						if (ItemManager.getTypeId(top) == CivData.WALL_SIGN) {
							ItemManager.setTypeId(top, CivData.AIR);
							ItemManager.setData(top, 0, true);
						}
				...
				centerY = CivCraft.mainWorld.getHighestBlockYAt(centerX, centerZ);

				// Determine if we should be a water good.
				ConfigTradeGood good;
				if (ItemManager.getBlockTypeIdAt(CivCraft.mainWorld, centerX, centerY - 1, centerZ) == CivData.WATER || ItemManager.getBlockTypeIdAt(CivCraft.mainWorld, centerX, centerY - 1, centerZ) == CivData.WATER_RUNNING) {
					good = pick.waterPick;
				} else {
					good = pick.landPick;
				}
				...
				TradeGoodPopulator.buildTradeGoodie(good, new BlockCoord(CivCraft.mainWorld, centerX, centerY, centerZ), CivCraft.mainWorld, true);
