	...
	public void processTrommelUpdate() {
		...
		for (int i = 0; i < trommel.skippedCounter+1; i++) {
			...
			for(ItemStack stack : contents) {
				...
				if (ItemManager.getTypeId(stack) == CivData.COBBLESTONE) {
					try {
					...
					if (rand1 < ((int)((trommel.getGravelChance(Mineral.CHROMIUM))*randMax))) {
						newItem = CustomMaterial.spawn(CustomMaterial.getCustomMaterial("mat_chromium_ore"));
				...
				if (ItemManager.getTypeId(stack) == CivData.STONE) {
					...
					if (this.trommel.getLevel() >= 2 && ItemManager.getData(stack) == 
						...
						try {
							this.updateInventory(Action.REMOVE, source_inv, ItemManager.createItemStack(CivData.STONE, (short) CivData.GRANITE, 1));
						} catch (InterruptedException e) {
							return;
						}
						...
						if (rand1 < ((int)((trommel.getGraniteChance(Mineral.CRYSTAL))*randMax))) {
							...
							if (rand2 < (randMax/10)) {
								int rand3 = rand.nextInt(randMax);
								 if (rand3 < (randMax/2)) {
										newItem = CustomMaterial.spawn(CustomMaterial.getCustomMaterial("mat_ionic_crystal_2"));
									} else {
										newItem = CustomMaterial.spawn(CustomMaterial.getCustomMaterial("mat_metallic_crystal_2"));
									}
							} else if (rand2 < (randMax/2)) {
								newItem = CustomMaterial.spawn(CustomMaterial.getCustomMaterial("mat_ionic_crystal_1"));
							} else {
								newItem = CustomMaterial.spawn(CustomMaterial.getCustomMaterial("mat_metallic_crystal_1"));
							}
						} else if (rand1 < ((int)((trommel.getGraniteChance(Mineral.REFINED_CHROMIUM))*randMax))) {
							newItem = CustomMaterial.spawn(CustomMaterial.getCustomMaterial("mat_forged_chromium"));
						} else if (rand1 < ((int)((trommel.getGraniteChance(Mineral.REFINED_TUNGSTEN))*randMax))) {
							newItem = CustomMaterial.spawn(CustomMaterial.getCustomMaterial("mat_forged_tungsten"));
						}else if (rand1 < ((int)((trommel.getGraniteChance(Mineral.CRYSTAL_FRAGMENT))*randMax))) {
							int rand2 = rand.nextInt(randMax);
							if (rand2 < (randMax/10)) {
								int rand3 = rand.nextInt(randMax);
								 if (rand3 < (randMax/2)) {
										newItem = CustomMaterial.spawn(CustomMaterial.getCustomMaterial("mat_ionic_crystal_fragment_2"));
									} else {
										newItem = CustomMaterial.spawn(CustomMaterial.getCustomMaterial("mat_metallic_crystal_fragment_2"));
									}
							} else if (rand2 < (randMax/2)) {
								newItem = CustomMaterial.spawn(CustomMaterial.getCustomMaterial("mat_ionic_crystal_fragment_1"));
							} else {
								newItem = CustomMaterial.spawn(CustomMaterial.getCustomMaterial("mat_metallic_crystal_fragment_1"));
							}
							...
							newItem = CustomMaterial.spawn(CustomMaterial.getCustomMaterial("mat_tungsten_ore"));
							...
							newItem = ItemManager.createItemStack(CivData.STONE, (short) CivData.POLISHED_GRANITE, 1);
					...
					if (this.trommel.getLevel() >= 3 && ItemManager.getData(stack) == 
						...
						try {
							this.updateInventory(Action.REMOVE, source_inv, ItemManager.createItemStack(CivData.STONE, (short) CivData.DIORITE, 1));
						} catch (InterruptedException e) {
							return;
						}
						// Attempt to get special resources
						...
						if (rand1 < ((int)((trommel.getDioriteChance(Mineral.CRYSTAL))*randMax))) {
							...
							newItem = ItemManager.createItemStack(CivData.STONE, (short) CivData.POLISHED_DIORITE, 1);
					...
					if (this.trommel.getLevel() >= 4 && ItemManager.getData(stack) == 
						...
						try {
							this.updateInventory(Action.REMOVE, source_inv, ItemManager.createItemStack(CivData.STONE, (short) CivData.ANDESITE, 1));
						} catch (InterruptedException e) {
							return;
						}
						// Attempt to get special resources
						...
						if (rand1 < ((int)((trommel.getAndesiteChance(Mineral.CRYSTAL))*randMax))) {
							...
							newItem = ItemManager.createItemStack(CivData.STONE, (short) CivData.POLISHED_ANDESITE, 1);
