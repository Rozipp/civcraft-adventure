...
public class TradeInventoryListener implements Listener {
	...
	public void markTradeValid(TradeInventoryPair pair) {
		pair.valid = true;
		pair.inv.addGuiItem(MY_SLOT_BUTTON, GuiItems.newGuiItem(ItemManager.createItemStack(CivData.WOOL, CivData.DATA_WOOL_GREEN, 1))//
		...
		pair.otherInv.addGuiItem(OTHER_SLOT_BUTTON, GuiItems.newGuiItem(ItemManager.createItemStack(CivData.WOOL, CivData.DATA_WOOL_GREEN, 1))//
	...
	public void markTradeInvalid(TradeInventoryPair pair) {
		pair.valid = false;
		pair.inv.addGuiItem(MY_SLOT_BUTTON, GuiItems.newGuiItem(ItemManager.createItemStack(CivData.WOOL, CivData.DATA_WOOL_RED, 1))//
				.setTitle(CivSettings.localize.localizedString("resident_tradeYourConfirm"))//
				.setLore(CivColor.Gold + CivSettings.localize.localizedString("resident_tradeClicktoConfirm")));

		pair.otherInv.addGuiItem(OTHER_SLOT_BUTTON, GuiItems.newGuiItem(ItemManager.createItemStack(CivData.WOOL, CivData.DATA_WOOL_RED, 1))//
	...
	private void completeTransaction(TradeInventoryPair pair, TradeInventoryPair otherPair) throws CivException {
		try {
			...
			for (int i = MY_SLOTS_START; i < MY_SLOTS_END; i++, k++) {
				...
				if (stack != null) myStuff.add(stack.getStack());
			}
			...
			for (int i = OTHERS_SLOTS_START; i < OTHERS_SLOTS_END; i++, k++) {
				...
				if (stack != null) theirStuff.add(stack.getStack());
			}
			/* Transfer any coins. */
	...
	public void onInventoryClose(InventoryCloseEvent event) {
		...
		TradeInventoryPair otherPair = tradeInventories.get(getTradeInventoryKey(pair.otherPlayer));
		if (otherPair != null) {
			GuiItem guiStack = GuiItems.newGuiItem()//
					.setTitle(pair.otherPlayer.getName() + " " + CivSettings.localize.localizedString("resident_tradeNotconfirmed"))//
