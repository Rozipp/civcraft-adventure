...
/* This here is an Axis Aligned Bounding Box. We use this to determine if structures may overlap each other during the build process. We may come up with other
 * uses for it as well such as checking for regions that a player is inside or something.
 * 
...
public class AABB {
	...
	public void showDebugBlocks(int mat, int mat2) {
		try {
			...
			ItemManager.sendBlockChange(dbgplayer,
					new Location(Bukkit.getWorld("world"), this.getPosition().getX(), this.getPosition().getY(), this.getPosition().getZ()), mat, 0);
			ItemManager.sendBlockChange(dbgplayer, new Location(Bukkit.getWorld("world"), this.getPosition().getX() + this.getExtents().getX(),
					this.getPosition().getY() + this.getExtents().getY(), this.getPosition().getZ() + this.getExtents().getZ()), mat2, (byte) 0);
	...
	public boolean overlaps(AABB other) {
		...
		return (Math.abs(t.getX()) < (getExtents().getX() + other.getExtents().getX()) && Math.abs(t.getY()) < (getExtents().getY() + other.getExtents().getY())
