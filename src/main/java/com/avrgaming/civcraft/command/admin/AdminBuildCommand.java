...
public class AdminBuildCommand extends MenuAbstractCommand {
	...
	public AdminBuildCommand(String perentCommand) {
		...
		add(new CustomCommand("showAllStructure").withDescription("adcmd_build_demolishDesc").withTabCompleter(new TownInWorldTaber()).withExecutor(new CustomExecutor() {
			@Override
			public void run(CommandSender sender, Command cmd, String label, String[] args) throws CivException {
				if (args.length < 1) {
					for (Structure structure : CivGlobal.getStructures()){
						CivMessage.send(sender, structure.getDisplayName() + ": " + structure.getCorner());
					}
					for (Title title : CivGlobal.getTitles()){
						CivMessage.send(sender, title.getDisplayName() + ": " + title.getCorner());
					}
					return;
				}
