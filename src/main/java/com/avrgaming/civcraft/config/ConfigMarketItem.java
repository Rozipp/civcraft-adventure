...
public class ConfigMarketItem {
	...
	public static void loadConfig(FileConfiguration cfg, Map<Integer, ConfigMarketItem> items) {
		...
		for (Map<?, ?> level : culture_levels) {
			...
			item.umid = (obj = level.get("custom_id")) == null ? null : (String) obj;
			if (item.umid == null) item.umid = (Integer) level.get("type_id") + ":" + (Integer) level.get("data");

			item.inital_value = (Integer) level.get("value");
			item.step = (obj = level.get("step")) == null ? STEP : (Integer) obj;
			Boolean stackable = (Boolean) level.get("stackable");
			if (stackable != null && stackable == false) item.stackable = stackable;
	...
	public static void init() throws SQLException {
		if (!SQL.hasTable(TABLE_NAME)) {
			String table_create = "CREATE TABLE " + SQL.tb_prefix + TABLE_NAME + " (" + "`ident` VARCHAR(64) NOT NULL," + "`buy_value` int(11) NOT NULL DEFAULT 105," + "`buy_bulk` int(11) NOT NULL DEFAULT 1,"
					+ "`sell_value` int(11) NOT NULL DEFAULT 95," + "`sell_bulk` int(11) NOT NULL DEFAULT 1," + "`buysell` int(11) NOT NULL DEFAULT 0," + "`bought` int(11) NOT NULL DEFAULT 0," + "`sold` int(11) NOT NULL DEFAULT 0,"
					+ "`last_action` mediumtext, " + "PRIMARY KEY (`ident`)" + ")";
	...
	public void buy(Resident resident, Player player, int amount) throws CivException {
		...
		ItemStack newStack = ItemManager.createItemStack(umid, amount);
	...
	public void sell(Resident resident, Player player, int amount) throws CivException {
		...
		if (!inv.contains(umid, amount)) throw new CivException(CivSettings.localize.localizedString("var_market_sell_notEnough", amount, this.name));
		...
		if (!inv.removeItem(umid, amount, true)) throw new CivException(CivSettings.localize.localizedString("var_market_sell_notEnough", amount, this.name));
