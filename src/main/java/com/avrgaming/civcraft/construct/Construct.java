...
public abstract class Construct extends SQLObject {
	...
	public void initDefaultTemplate(Location location) throws CivException {
		if (getInfo().require_floor != null) {
			...
			if (cci == null) throw new CivException("I dont no Require_floor = " + getInfo().require_floor);
			else {
				Title title =  getTownOwner().BM.getNearestTitle(location);
				if (title == null || !getInfo().require_floor.equals(title.getInfo().id) || !CivGlobal.getConstructsFromChunk(new BlockCoord(location)).contains(title)) throw new CivException("Это здание надо строит в " + cci.displayName);
				int slot = title.getNextSlot(getInfo());
				String tplPath = Template.getTemplateFilePath(getInfo().template_name, title.getDirection(slot), null);
				this.setTemplate(Template.getTemplate(tplPath));
				this.setCorner(title.getBlockCoordSlot(slot));
			}
		} else{
		...
		TaskMaster.asyncTask(() -> {
			...
			for (int y = 0; y < tpl.size_y; y++) {
				for (SimpleBlock sb : tpl.blocks.get(y)) {
					...
					construct.addConstructBlock(bc.clone(), (y != 0));
				}
			}
		}, 20);
	}
		...
		for (SimpleBlock sb : tpl.commandBlockRelativeLocations) {
			...
			case "/chest":
				...
				if (ItemManager.getTypeId(block) != CivData.CHEST) {
					byte chestData = Direction.convertSignDataToChestData((byte) sb.getData());
