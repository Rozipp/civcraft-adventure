...
public class Template {
	...
	public int size_y;
	public int size_z;
	static int blocksPerTick = 20000;
	...
	public Template(String filepath) throws IOException, CivException {
		this.filepath = filepath;
		this.direction = Direction.newDirection(filepath);
		load_template();
	}
	...
	public static String getTemplateFilePath(Location loc, ConfigConstructInfo info, String theme) throws CivException {
		...
		return Template.getTemplateFilePath(info.template_name, Direction.newDirection(loc), theme);
	}

	public static String getTemplateFilePath(String template_name, Direction direction, String theme) throws CivException {
		...
		if (direction != null && direction != Direction.none) ss = ss + "_" + direction;
	...
	public static String getTemplateFilePath(Location loc, String template_name, String theme) throws CivException {
		return Template.getTemplateFilePath(template_name, Direction.newDirection(loc), theme);
	}
				...
				for (int y = this.size_y - 1; y >= 0; y--)
					for (int x = 0; x < this.size_x; x++) {
						for (int z = 0; z < this.size_z; z++) {
							...
							SimpleBlock sb = (isPutBlock) ? new SimpleBlock(CivSettings.scaffoldingType) : new SimpleBlock(CivData.AIR, 0);
				...
				for (int y = 0; y < this.size_y; y++) {
					for (int x = 0; x < this.size_x; x++) {
						for (int z = 0; z < this.size_z; z++) {
							...
							if (ItemManager.getType(b) == ItemManager.getMaterial(CivSettings.scaffoldingType))//
