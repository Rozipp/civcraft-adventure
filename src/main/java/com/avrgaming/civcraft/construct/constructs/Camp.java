...
public class Camp extends Construct {
	...
	public void saveNow() throws SQLException {
		...
		hashmap.put("owner_name", this.getResidentOwner().getUuid().toString());
	...
	public void processValidateCommandBlockRelative() {
		...
		for (SimpleBlock sb : tpl.commandBlockRelativeLocations) {
			...
			case "/growth":
				...
				if (annexLevel.getOrDefault(annex, 0) >= level) {
					...
					for (int i = 0; i < Camp.offset.length; i++)
						for (int j = 1; j < 5; j++) {
							this.memberProtectionBlocks.add(absCoord.getRelative(Camp.offset[i][0], j, Camp.offset[i][1]));
						}
			...
			case "/sifter": // /chest,id:sifterresult,level:1,annex:sifter, для совместимости
				...
				annex = "sifter";
				if (annexLevel.getOrDefault(annex, 0) >= level) {
					int idchest = Integer.parseInt(sb.keyvalues.get("id"));
					switch (idchest) {
					case 0:
					...
					byte data2 = Direction.convertSignDataToChestData((byte) sb.getData());
			...
			case "/firefurnace":
				...
				byte data = Direction.convertSignDataToChestData((byte) sb.getData());
			...
			case "/door":
				...
				byte topData = 0x8;
				byte bottomData = 0x0;
				byte doorDirection = Direction.newDirection(sb).getDoorData();
	...
	public void processSignAction(Player player, ConstructSign sign, PlayerInteractEvent event) {
		...
		if (!this.getResidentOwner().equals(resident)) {
	...
	public void processFirepoints() {
		...
		while (mInv.contains(Material.COAL.name(),Camp.coal_per_firepoint) && this.firepoints < Camp.maxFirePoints) {
			try {
				mInv.removeItem(Material.COAL, Camp.coal_per_firepoint, true);
				this.firepoints++;
				coutAddFirePoints++;
	...
	public void processLonghouse(CivAsyncTask task) {
		...
		this.getResidentOwner().getTreasury().deposit(coins);
...
//	}
	...
	public void onControlBlockHit(ControlPoint cp, Player player) {
		...
		CivMessage.send(player, "§7" + CivSettings.localize.localizedString("camp_hitControlBlock") + "(" + cp.getHitpoints() + " / " + cp.getMaxHitpoints() + ")");
	...
	public void onControlBlockDestroy(ControlPoint cp, Player player) {
		...
		ItemManager.setTypeId(cp.getCoord().getLocation().getBlock(), 0);
		...
		if (cp != null) {
			...
			if (now.after(this.getNextRaidDate())) {
				if (!cp.isDestroyed()) {
					...
					CivMessage.send(player, "§c" + CivSettings.localize.localizedString("camp_controlBlockAlreadyDestroyed"));
				}
			} else {
				SimpleDateFormat sdf = CivGlobal.dateFormat;
				CivMessage.send(player, "§c" + CivSettings.localize.localizedString("camp_protectedUntil") + " " + sdf.format(this.getNextRaidDate()));
			}
		}
	}
	...
	public void purchaseUpgrade(ConfigCampUpgrade upgrade) throws CivException {
		Resident owner = this.getResidentOwner();
