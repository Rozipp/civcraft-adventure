...
public class TownBuildableManager {
	...
	public void addBuildable(Buildable buildable) {
		...
		if (buildable.getInfo().require_floor != null) {
			Title title = getNearestTitle(buildable.getCenterLocation());
			try {
				title.setBuildableToSlot(title.getNextSlot(buildable.getInfo()), buildable);
			} catch (CivException e) {
				e.printStackTrace();
			}
		}
	...
	public void removeBuildable(Buildable buildable) {
		...
		if (buildable instanceof  Structure) {
			...
			this.titles.remove(buildable.getCorner());
		}
		else if (buildable instanceof  Wonder) {
