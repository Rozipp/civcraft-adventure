...
public class MultiInventory {
	...
	private int removeItemFromInventory(Inventory inv, String umid, int amount, Boolean direct) {
		int removed = 0;
	...
	public boolean removeItem(String umid, int amount, Boolean direct) throws CivException {
		ArrayList<ItemInvPair> toBeRemoved = new ArrayList<>();

		int count = amount;
		for (Inventory inv : invs) {
			for (ItemStack item : inv.getContents()) {
				if (!ItemManager.isCorrectItemStack(item, umid)) continue;
				...
				if (item.getAmount() > count) {
					toBeRemoved.add(new ItemInvPair(inv, umid, count));
					count = 0;
					break;
				} else
					if (item.getAmount() == count) {
						...
						toBeRemoved.add(new ItemInvPair(inv, umid, item.getAmount()));
		...
		for (ItemInvPair invPair : toBeRemoved) {
			...
			totalActuallyRemoved += removeItemFromInventory(inv, invPair.umid, invPair.amount, direct);
		}
	...
	public boolean removeItemStackSync(ItemStack item, Boolean direct) throws CivException {
		return removeItem(ItemManager.getUMid(item), item.getAmount(), direct);
	}

	public boolean removeItem(Material mat, int amount, Boolean sync) throws CivException {
		return removeItem(mat.name(), amount, sync);
	}

	public boolean contains(String umid, int amount) {
		int count = 0;
		for (Inventory inv : invs) {
			for (ItemStack item : inv.getContents()) {
	...
	public int hasEnough(SourceItem sourceItem, int neadFound) {
		...
		int count = neadFound * sourceItem.count;
