...
public class BuildStructureList extends GuiAction {
...
//			int type = ItemManager.getMaterialId(Material.ANVIL);
...
//			ItemStack is;
//			if (!resident.hasTown()) {
//				is = LoreGuiItem.build(info.displayName, ItemManager.getMaterialId(Material.BEDROCK), 0, CivColor.Rose+"Must belong to a town to build structures.");
//			} else {
//				if (!resident.getCiv().hasTechnology(info.require_tech)) {
//					ConfigTech tech = CivSettings.techs.get(info.require_tech);
//					is = LoreGuiItem.build(info.displayName, ItemManager.getMaterialId(Material.BEDROCK), 0, CivColor.Rose+"Requires: "+tech.name);
//				} else {
