...
public class CraftingHelpRecipe extends GuiInventory {
	...
	public static final int START_OFFSET = GuiItem.INV_ROW_COUNT + 3;
	...
	public GuiItem getIngredItem(ConfigIngredient ingred) {
		...
		if (ingred.custom_id == null) {
			...
			entryStack = GuiItem.newGuiItem(ItemManager.createItemStack(ingred.type_id, (short) ingred.data, 1))//
			...
			name = cmat.getName();
			entryStack = GuiItem.newGuiItem(CraftableCustomMaterial.spawn(cmat));
			if (cmat.isCraftable()) {
				entryStack.addLore(CivSettings.localize.localizedString("loreGui_recipes_clickForRecipe"))//
						.setOpenInventory("CraftingHelpRecipe", cmat.getId());
			} else
	...
	public void buildCraftTableBorder(GuiInventory recInv) {
		int offset = 2;
		GuiItem item = GuiItem.newGuiItem().setTitle("Craft Table Border").setMaterial(Material.WORKBENCH);
		for (int y = 0; y <= 4; y++) {
			for (int x = 0; x <= 4; x++) {
				if (x == 0 || x == 4 || y == 0 || y == 4) {
					recInv.addGuiItem(offset + (y * GuiItem.INV_ROW_COUNT) + x, item);
				}
			}
		}
	}
		...
		if (craftMat.getConfigMaterial().required_tech != null) {
			...
			if (tech != null) {
				stack = (resident.hasTown() && resident.getCiv().hasTechnologys(craftMat.getConfigMaterial().required_tech)) ? //
						GuiItem.newGuiItem().setTitle(CivSettings.localize.localizedString("loreGui_recipes_requiredTech")).setMaterial(Material.EMERALD_BLOCK).setLore(tech.name) : //
						GuiItem.newGuiItem().setTitle(CivSettings.localize.localizedString("loreGui_recipes_requiredTech")).setMaterial(Material.REDSTONE_BLOCK).setLore(tech.name);
				recInv.addGuiItem(offset, stack);
			}
		}

		stack = (craftMat.isShaped()) ? //
				GuiItem.newGuiItem().setTitle(CivSettings.localize.localizedString("loreGui_recipes_shaped")).setMaterial(Material.HOPPER) : //
				GuiItem.newGuiItem().setTitle(CivSettings.localize.localizedString("loreGui_recipes_unshaped")).setMaterial(Material.COAL);
		offset += GuiItem.INV_ROW_COUNT;
		recInv.addGuiItem(offset, stack);
	}
